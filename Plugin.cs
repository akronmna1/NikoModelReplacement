...
namespace niko
{
    [BepInPlugin("staff.niko", "Niko", "0.1.0")]
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        {
                ModelReplacementAPI.RegisterModelReplacementOverride(typeof(MRNIKO));

            }
            if (enableModelAsDefault.Value)
            {
                ModelReplacementAPI.RegisterModelReplacementDefault(typeof(MRNIKO));

            }
            var commaSepList = suitNamesToEnableModel.Value.Split(',');
            foreach (var item in commaSepList)
            {
                ModelReplacementAPI.RegisterSuitModelReplacement(item, typeof(MRNIKO));
            }
                

            Harmony harmony = new Harmony("staff.niko");
            harmony.PatchAll();
            Logger.LogInfo($"Plugin {"staff.niko"} is loaded!");
        }
    }
    public static class Assets
    {
        // Replace mbundle with the Asset Bundle Name from your unity project 
        public static string mainAssetBundleName = "niko";
